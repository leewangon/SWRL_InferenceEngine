...
//      deliberationRDD.collect().foreach(println)
      deliberationRDD.map(tripleList => (tripleList(subjIndex(0)).productElement(subjIndex(1)).toString, pred, tripleList(objIndex(0)).productElement(objIndex(1)).toString ))
    }
    else if(subj.contains("?") && !obj.contains("?")){
      deliberationRDD.map(tripleList => (tripleList(subjIndex(0)).productElement(subjIndex(1)).toString, pred, obj))
    }
    else{
//      deliberationRDD.collect().foreach(println)
      deliberationRDD.map(tripleList => (subj, pred, tripleList(objIndex(0)).productElement(objIndex(1)).toString ))
    }
  }
